...
func TestBinaryInstallStrategyDefaults(t *testing.T) {
	strategy := &cfov1.BinaryInstallStrategy{
...
func TestBinaryComponentWithAllFields(t *testing.T) {
	component := cfov1.BinaryComponent{
		Version:       "1.7.24",
		Type:          cfov1.ComponentTypeBinary,
		Path:          "/opt/capbm/binaries/containerd",
		Architectures: []string{"amd64", "arm64"},
		Files: cfov1.BinaryFiles{
		...
		UpgradeStrategy: &cfov1.BinaryUpgradeStrategy{
			Type:          "Rolling",
			MaxConcurrent: 1,
			Timeout:       &metav1.Duration{Duration: 600000000000},
			RetryCount:    3,
		...
		Upgrade: &cfov1.ComponentUpgradeConfig{
			Backup: cfov1.ComponentBackupConfig{
				Enabled: true,
				Config: []cfov1.BackupItem{
					{Path: "/etc/containerd/config.toml", Type: "file"},
				},
			},
			Rollback: cfov1.ComponentRollbackConfig{
				Script:  "scripts/rollback-containerd.sh",
				Timeout: &metav1.Duration{Duration: 300000000000},
			},
			HealthCheck: cfov1.ComponentHealthCheckConfig{
...
func TestKubernetesComponentWithAllFields(t *testing.T) {
	component := cfov1.KubernetesComponent{
		Version: "v1.31.0",
		Type:    cfov1.ComponentTypeBinary,
		Path:    "/opt/capbm/binaries/kubernetes",
		Platforms: map[string]cfov1.K8SPlatform{
			"ubuntu": {
				Architectures: []string{"amd64", "arm64"},
...
func TestReleaseImageWithBinaryComponents(t *testing.T) {
	releaseImage := cfov1.ReleaseImage{
		Spec: cfov1.ReleaseImageSpec{
			...
			Components: cfov1.ReleaseComponentVersions{
				Kubernetes: cfov1.KubernetesComponent{
					Version: "v1.31.0",
					Type:    cfov1.ComponentTypeBinary,
					InstallStrategy: &cfov1.BinaryInstallStrategy{
						Timeout:     &metav1.Duration{Duration: 600000000000},
				...
				Containerd: cfov1.BinaryComponent{
					Version: "1.7.24",
					Type:    cfov1.ComponentTypeBinary,
					InstallStrategy: &cfov1.BinaryInstallStrategy{
						Timeout:     &metav1.Duration{Duration: 300000000000},
