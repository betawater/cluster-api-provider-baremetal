...
func (t *StatusTracker) buildImportedImageStatus(releaseImage *cfov1.ReleaseImage, job *batchv1.Job) []cfov1.ImportedImageStatus {
	var statuses []cfov1.ImportedImageStatus
	...
	for _, comp := range components {
		...
		for _, image := range comp.imageList {
			...
			status := cfov1.ImportedImageStatus{
				Component: comp.name,
				Image:     imageName,
				TargetRef: targetRef,
			}
