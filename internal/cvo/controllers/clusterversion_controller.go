...
import (
	...
	"github.com/BetaWater/cluster-api-provider-baremetal/internal/cvo/addon"
	"github.com/BetaWater/cluster-api-provider-baremetal/internal/cvo/upgrader"
)
...
func (r *ClusterVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	cv.Status.History = prependHistory(cv.Status.History, commonv1.UpdateHistory{
		State:       commonv1.PartialUpdate,
	...
	if len(cv.Status.History) > 0 {
		cv.Status.History[0].State = commonv1.CompletedUpdate
		now := metav1.Now()
...
func (r *ClusterVersionReconciler) fetchReleaseImage(ctx context.Context, cv *cfov1.ClusterVersion) (*cfov1.ReleaseImage, error) {
	...
	cv.Status.Desired = commonv1.Release{
...
func (r *ClusterVersionReconciler) initComponentStatus(releaseImage *cfov1.ReleaseImage) []commonv1.ComponentStatus {
	var status []commonv1.ComponentStatus

	// Add containerd
	if releaseImage.Spec.Components.Containerd.Version != "" {
		status = append(status, commonv1.ComponentStatus{
			Name:          "containerd",
...
func findAddonDefByName(releaseImage *cfov1.ReleaseImage, name string) *commonv1.AddonDefinition {
...
func prependHistory(history []commonv1.UpdateHistory, entry commonv1.UpdateHistory) []commonv1.UpdateHistory {
	if len(history) > 0 && history[0].Version == entry.Version && history[0].State == commonv1.PartialUpdate {
		return history
	}
	return append([]commonv1.UpdateHistory{entry}, history...)
}
