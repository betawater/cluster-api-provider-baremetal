...
所有组件（containerd/kubeadm/kubelet/kubectl/CNI/CSI）使用统一的 `INSTALL_SOURCE` 参数控制资源获取方式：

| 模式 | INSTALL_SOURCE | 资源来源 | 适用场景 |
|------|---------------|---------|---------|
| 在线 | `online` | 公网 (github.com, pkgs.k8s.io, helm repos) | 开发测试，有外网 |
| HTTP | `http` | 内部版本包 HTTP 服务器 | 内网环境，有内部服务器 |
| 本地 | `local` | 目标机器本地预置文件 | 完全隔离环境 |

**核心设计原则**: 通过 `fetch_resource()` 函数抽象资源获取逻辑，所有安装脚本共享同一套安装逻辑，仅资源获取方式不同。
...
##### 6.16.4.2 HAProxy 注册流程 (Runtime API 方式)
...
##### 6.16.4.3 HAProxy 注册脚本 (Runtime API)
...
##### 6.16.4.4 HAProxy 注册脚本 (SSH + 配置文件方式)
...
#### 6.16.6 ClusterClass 变量和 Patches
...
##### 6.16.7.2 Keepalived + HAProxy 方式 (无外部 LB)
...
CAPBM 需要在组件安装流程中集成 CNI/CSI 的安装和升级能力，支持三种安装源模式：

| 模式 | INSTALL_SOURCE | 资源来源 | 适用场景 |
|------|---------------|---------|---------|
| 在线 | `online` | 公网 (github.com, helm repos) | 开发测试，有外网 |
| HTTP | `http` | 内部版本包 HTTP 服务器 | 内网环境，有内部服务器 |
| 本地 | `local` | 目标机器本地预置文件 | 完全隔离环境 |

**核心设计原则**: 所有组件（containerd/kubeadm/kubelet/kubectl/CNI/CSI）使用统一的 `INSTALL_SOURCE` 参数控制资源获取方式，通过 `fetch_resource()` 函数抽象资源获取逻辑。
...
所有安装脚本都包含 `fetch_resource()` 函数作为资源获取的统一入口：
...
**版本包目录结构 (HTTP/Local 模式使用)**:

```
release-server/ 或 local-path/
├── k8s/
│   ├── v1.31/
│   │   ├── kubelet-1.31.0.deb
│   │   ├── kubeadm-1.31.0.deb
...
##### 6.17.4.1 统一安装脚本 (三种模式)

所有 CNI 安装脚本使用统一的 `INSTALL_SOURCE` 参数控制资源获取方式：
...
| 维度 | online | http | local |
|------|--------|------|-------|
| 资源来源 | github.com, helm repos | 内部 HTTP 服务器 | 目标机器本地路径 |
| 网络要求 | 外网可达 | 内网 HTTP 可达 | 无网络要求 |
| 适用场景 | 开发测试 | 生产内网 | 完全隔离环境 |
| 速度 | 取决于外网 | 内网高速 | 本地磁盘速度 |
| 版本控制 | 实时获取 | 版本包控制 | 版本包控制 |
kubeadm init 执行完成
    │
    ▼
┌─────────────────────────────────────────────────┐
...
##### 6.17.5.1 统一安装脚本 (三种模式)

CSI 安装脚本同样使用统一的 `INSTALL_SOURCE` 参数：
...
load_csi_images() {
    case "$INSTALL_SOURCE" in
        ...
        http|local)
            ...
            fetch_resource "csi/${CSI_DRIVER}-images.tar" "$images_archive"
...
case "$CSI_DRIVER" in
    ceph-csi)
        [ "$INSTALL_MODE" = "Helm" ] && install_ceph_csi_helm ;;
...
#### 6.17.8 ClusterClass 变量和 Patches
...
##### 6.17.10.1 标准集群 (Calico CNI + Local-CSI)
...
##### 6.17.10.2 高性能集群 (Cilium CNI + Ceph-CSI)
